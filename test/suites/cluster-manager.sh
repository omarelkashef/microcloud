...
PYEOF"
  ...
  echo "==> Reject lxd_url values without a host"
  lxc exec micro01 --env TEST_CONSOLE=0 -- microcloud cluster-manager set lxd_url /path 2>&1 | grep -F "Invalid lxd_url: URL \"/path\" is missing a scheme or host" -q
  lxc exec micro01 --env TEST_CONSOLE=0 -- microcloud cluster-manager get lxd_url | grep -F "https://example.com:8443" -q
  lxc exec micro01 --env TEST_CONSOLE=0 -- microcloud cluster-manager set lxd_url http:/path 2>&1 | grep -F "Invalid lxd_url: URL \"http:/path\" is missing a scheme or host" -q
  lxc exec micro01 --env TEST_CONSOLE=0 -- microcloud cluster-manager get lxd_url | grep -F "https://example.com:8443" -q

  echo "==> Reject lxd_url values with a port but no hostname"
  lxc exec micro01 --env TEST_CONSOLE=0 -- microcloud cluster-manager set lxd_url https://:8443 2>&1 | grep -F "Invalid lxd_url: URL \"https://:8443\" is missing a scheme or host" -q
  lxc exec micro01 --env TEST_CONSOLE=0 -- microcloud cluster-manager get lxd_url | grep -F "https://example.com:8443" -q
