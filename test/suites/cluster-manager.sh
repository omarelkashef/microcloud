...
PYEOF"
  ...
  echo "==> LXD UI URL config for cluster manager"
  ...
  lxc exec micro01 --env TEST_CONSOLE=0 -- microcloud cluster-manager set lxd_url ftp://example.com 2>&1 | grep -F "Invalid lxd_url: Invalid value \"ftp://example.com\": URL scheme must be http or https" -q

  echo "==> Accept http and https lxd_url values"
  lxc exec micro01 --env TEST_CONSOLE=0 -- microcloud cluster-manager set lxd_url https://example.com/ui
  lxc exec micro01 --env TEST_CONSOLE=0 -- microcloud cluster-manager get lxd_url | grep "https://example.com/ui" -q
  lxc exec micro01 --env TEST_CONSOLE=0 -- microcloud cluster-manager show | grep "lxd_url: https://example.com/ui" -q

  lxc exec micro01 --env TEST_CONSOLE=0 -- microcloud cluster-manager set lxd_url http://example.com/ui
  lxc exec micro01 --env TEST_CONSOLE=0 -- microcloud cluster-manager get lxd_url | grep "http://example.com/ui" -q

  echo "==> Accept lxd_url values with an IP host"
  lxc exec micro01 --env TEST_CONSOLE=0 -- microcloud cluster-manager set lxd_url https://192.168.1.10:8443/ui
  lxc exec micro01 --env TEST_CONSOLE=0 -- microcloud cluster-manager get lxd_url | grep "https://192.168.1.10:8443/ui" -q
