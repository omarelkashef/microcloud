...
// LXDURLKey is the key for the LXD UI URL configuration.
...
// LoadLXDURL loads the LXD UI URL configuration from the database.
func LoadLXDURL(state types.State, ctx context.Context, clusterManagerId int64) (string, error) {
	uiUrlConfig, err := LoadClusterManagerSingleConfig(state, ctx, clusterManagerId, LXDURLKey)
	if err != nil {
		return "", err
	}

	if uiUrlConfig == nil {
		return "", api.StatusErrorf(http.StatusNotFound, "UI URL not found")
	}

	return uiUrlConfig.Value, nil
}
